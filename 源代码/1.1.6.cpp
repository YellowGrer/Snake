...
char choT[2][10][1000] = { {"开始游戏", "游戏设置", "操作提示", "当前最高得分："}, {"每格移动时间", "物品生成时间", "物品存在时间", "版本：1.1.6"} }; //显示选项用
...
void game()
{
	...
	//更改蛇头方向
	else if ((GetAsyncKeyState('D') & 0x8000) && (sna[0][2] == 1 || sna[0][2] == 3))
	{
		sna[0][2] = 0;
	}
	else if ((GetAsyncKeyState('S') & 0x8000) && (sna[0][2] == 0 || sna[0][2] == 2))
	{
		sna[0][2] = 1;
	}
	else if ((GetAsyncKeyState('A') & 0x8000) && (sna[0][2] == 1 || sna[0][2] == 3))
	{
		sna[0][2] = 2;
	}
	else if ((GetAsyncKeyState('W') & 0x8000) && (sna[0][2] == 0 || sna[0][2] == 2))
	{
		sna[0][2] = 3;
	}
	//退出
	...
	for (int i = 0; i <= 1; i++)
	{
		...
		if (iteT[i] == 0)
		{
			if (ite[i][1] < 0)
			{
				...
				if ((abl > 0) && ((ite[i][0] == 0 && bonG > 0) || ite[i][0] != 0))
				{
