...
char choT[4][30][100] = { {"开始游戏", "游戏设置", "操作提示", "高分记录"},
    {"移动速度", "物品生成时间", "物品存在时间", "恢复默认", "版本：1.3.4"},
    {"多种方块模式", "一直变长模式"},
...
char tipT[4][30][100] = { {"在菜单页操作时：", "空格", "以确认选择", "W", "以切换上一个选项", "S","以切换下一个选项", "Esc", "以关闭游戏"},
    ...
    {"游戏中有以下方块：", "食物", "蛇长+，分数+", "奖励", "分数+", "厄运", "分数-", "幸运", "分数随机+或-", "减肥", "给予减肥状态", "礼包", "生成5个礼物", "礼物", "分数+"},
...
void show(int m)
{
    ...
    else if (m == 6)
    {
        //标题
        settextstyle(80, 0, "Consolas");
        outtextxy(W * 15 - 118, 100, "Records");
        //选项
        showChoice(3, 3);
        sprintf_s(tex, "%d", scoM[0]);
        outtextxy(W * 15 - 200, H * 15 + 5, tex);
        sprintf_s(tex, "%d", scoM[1]);
        outtextxy(W * 15 - 200, H * 15 + 65, tex);
    }
...
{
    int x0 = obsWI[0], y0 = obsWI[1], x1, y1, x2, y2;
    if (d == 0)
...
void wanderingObstacleUpdate()
{
    ...
    if (obsWI[3] == 3)
    {
        ...
        for (int i = 0; !wanderingObstacleMoveJudge(obsWI[2]) && i < 3; i++)
            obsWI[2] = (obsWI[2] + 1) % 4;
        if (wanderingObstacleMoveJudge(obsWI[2]))
        {
            for (int i = obsWI[0]; i <= obsWI[0] + 1; i++)
                for (int j = obsWI[1]; j <= obsWI[1] + 1; j++)
            ...
            for (int i = obsWI[0]; i <= obsWI[0] + 1; i++)
                for (int j = obsWI[1]; j <= obsWI[1] + 1; j++)
                    if (map[(i + W) % W][(j + H) % H] == 0 && obsWE[i - obsWI[0]][j - obsWI[1]])
                        map[(i + W) % W][(j + H) % H] = 8;
        }
    }
}
...
void game()
{
    ...
    else if (blo == 8)
    {
        ...
        {
            scoN += len + 10;
            if (sna[0][0] >= obsWI[0] && sna[0][0] <= obsWI[0] + 1 && sna[0][1] >= obsWI[1] && sna[0][1] <= obsWI[1] + 1)
            {
                obsWE[sna[0][0] - obsWI[0]][sna[0][1] - obsWI[1]] = 0;
                bool fla = true;
                for (int i = 0; i <= 1; i++)
                    for (int j = 0; j <= 1; j++)
                        if (obsWE[i][j])
