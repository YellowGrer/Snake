...
{
    fillrectangle(x * 30, y * 30, x * 30 + 30, y * 30 + 30);
}
...
void generateItem()
{
    for (int i = 0; i <= 1; i++)
    {
        ...
        if (iteT[i] == 0)
        {
            if (!(map[ite[i][1]][ite[i][2]] >= 2 && map[ite[i][1]][ite[i][2]] <= 6))
            {
                generatePrepare();
                if (abl > 0)
                {
