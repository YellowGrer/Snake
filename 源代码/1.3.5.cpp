...
char choTex[5][30][100] = { {"开始游戏", "游戏设置", "操作提示", "高分记录", "游戏图鉴"},
    {"移动速度", "物品生成时间", "物品存在时间", "恢复默认", "版本：1.3.5", "作者：MathYellow"},
    {"多种方块模式", "一直变长模式"},
    ...
    else if (m == 7)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if (boo[i])
            {
                ...
                outtextxy(i % 6 * 120 + 86 - textLength(bloNam[i]) * 4, i / 6 * 150 + 360, bloNam[i]);
            }
            else
            {
                ...
                settextstyle(20, 0, "Consolas");
                char c[] = "Locked";
                outtextxy(i % 6 * 120 + 86 - textLength(c) * 4, i / 6 * 150 + 360, c);
            }
        }
    }
