...
char choTex[5][30][100] = { {"开始游戏", "游戏设置", "操作提示", "高分记录", "游戏图鉴"},
    {"移动速度", "物品生成时间", "物品存在时间", "恢复默认", "版本：1.3.6", "作者：MathYellow"},
    {"充满冒险的模式", "一直进食的模式"},
...
int obsNum;           //屏障生成数量
int obsSizNum;        //静止屏障大小种类数量
int obs[50][2];       //静止屏障大小
int wanObsInf[4];     //游荡屏障信息
bool wanObsExi[2][2];  //游荡屏障各部分是否存在
int fliObsInf[4];     //闪烁屏障信息
bool fliObsExi[max(W, H)];  //闪烁屏障各部分是否存在
...
//闪烁屏障生成函数
void generateFlickeringObstacle()
{
    abl = W + H;
    bool genW[W], genH[H];
    for (int i = 0; i < W; i++)
...
void flickeringObstacleUpdate()
{
    ...
    if (!fliObsInf[3])
    {
        ...
        if (fliObsInf[2])
        {
            ...
            {
                for (int i = 0; i < W; i++)
                    if (map[i][fliObsInf[1]] == 8 && fliObsExi[i])
                        map[i][fliObsInf[1]] = 0;
            }
            else
                for (int i = 0; i < H; i++)
                    if (map[fliObsInf[1]][i] == 8 && fliObsExi[i])
                        map[fliObsInf[1]][i] = 0;
        ...
        else
        {
            ...
            {
                for (int i = 0; i < W; i++)
                    if (map[i][fliObsInf[1]] == 0 && fliObsExi[i])
                        map[i][fliObsInf[1]] = 8;
                    else
                        fliObsExi[i] = false;
            }
                ...
                for (int i = 0; i < H; i++)
                    if (map[fliObsInf[1]][i] == 0 && fliObsExi[i])
                        map[fliObsInf[1]][i] = 8;
                    else
                        fliObsExi[i] = false;
        }
...
void game()
{
    ...
    else if (blo == 8)
    {
        if (buf[0])
        {
            if (sna[0][0] >= wanObsInf[0] && sna[0][0] <= wanObsInf[0] + 1 && sna[0][1] >= wanObsInf[1] && sna[0][1] <= wanObsInf[1] + 1)  //游荡屏障
            {
                scoNow += len;
            ...
            else if ((fliObsInf[0] == 0 && sna[0][0] == fliObsInf[1] && fliObsExi[sna[0][1]]) || (fliObsInf[0] == 1 && sna[0][1] == fliObsInf[1] && fliObsExi[sna[0][0]]))  //闪烁屏障
            {
                scoNow += len / 2;
                fliObsInf[3] = max(1, fliObsInf[3] - 5);
                bool fla = true;
                if (fliObsInf[0] == 0)
                {
                    fliObsExi[sna[0][1]] = false;
                    for (int i = 0; i < H; i++)
                        if (fliObsExi[i])
                            fla = false;
                }
